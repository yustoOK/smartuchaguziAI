...
def load_and_preprocess_data(file_path):
    ...
    # Apply SMOTE to oversample fraud cases in training data
    smote = SMOTE(random_state=42, sampling_strategy=0.5)  # Aim for 1:2 fraud-to-normal ratio
    X_train, y_train = smote.fit_resample(X_train, y_train)
    
    return X_train, X_val, X_test, y_train, y_val, y_test, features

def build_model(input_dim):
    # Simplified model for small dataset
    model = Sequential([
...
def train_model(X_train, X_val, y_train, y_val):
    # Adjusted class weights after SMOTE (less aggressive due to oversampling)
    class_weights = {0: 1.0, 1: 2.0}  # Reflects SMOTE's 1:2 ratio
